...
class CB_Worker(mp.Process):
    def __init__(self, inQ, outQ, G, topic_correlation, msg_list, k, index_id):
...
class UC_Worker(mp.Process):
    def __init__(self, inQ, outQ, G, topic_correlation, msg_list, k, index_id):
        super(UC_Worker, self).__init__(target=self.start)
        self.inQ = inQ
        self.outQ = outQ
        self.G = G
        self.topic_correlation = topic_correlation
    ...
    def run(self):
        while True:
            ...
            users_pre_matrix = np.array([self.G.nodes[user]['sendCount']['total'] for user in user_ids]).reshape((-1, topic_num))
            all_users_pre_matrix = np.array([self.G.nodes[user]['sendCount']['total'] for user in self.G.nodes()]).reshape((-1, topic_num))
            similarity_matrix = users_pre_matrix.dot(all_users_pre_matrix.T) / (np.linalg.norm(users_pre_matrix, axis=1).reshape(-1, 1) * np.linalg.norm(all_users_pre_matrix, axis=1))
            similarity_matrix[np.isneginf(similarity_matrix)] = 0
...
def create_CB_workers(num, G, topic_correlation, msg_list, k, index_id):
    worker = []
    for i in range(num):
